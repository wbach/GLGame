...
void CEntityRenderer::Render(shared_ptr<CScene>& scene)
{
	...
	m_EntityShader.LoadLightNumber(static_cast<float>(scene->GetLights().size()));

	m_EntityShader.LoadSkyColour(0.6f, 0.6f, 0.8f);
	m_EntityShader.LoadClipPlaneVector(glm::vec4(0, 1, 0, 100000));
	m_EntityShader.LoadViewDistance(625.0f);
	...
	{		
		for (shared_ptr<CEntity> entity : terr.m_TerrainEntities)
		{
...
void CEntityRenderer::RenderEntity(shared_ptr<CEntity>& entity, CModel& model)
{
	for (CMesh& mesh : model.m_Meshes) 
	{
		...
		for (glm::mat4& mat : entity->GetTransformMatrixes())
		{
			...
			m_EntityShader.LoadNumberOfRows(static_cast<float>(mesh.material.numberOfRows));
