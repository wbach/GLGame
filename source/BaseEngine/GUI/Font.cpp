...
void CFont::Init(std::string file_name, glm::vec2 window_size)
{
...
void CFont::Print(const int& x, const int& y, const char *fmt) const
{
	...
	char		_text[256];								// Holds Our String
	va_list		ap;										// Pointer To List Of Arguments



	//if (text.size() <= 0)									// If There's No Text
	//	return;												// Do Nothing
	//else
	{
		va_start(ap, fmt);									// Parses The String For Variables
		vsprintf_s(_text, fmt, ap);						// And Converts Symbols To Actual Numbers
...
void CFont::CreateList(FT_Face face, char ch)
{
	...
	/*glScalef(static_cast<GLfloat>(bitmap.width), static_cast<GLfloat>(bitmap.rows), 1.f);
	Utils::SimpleRenderVao(m_QuadVao, m_QuadIndicesSize, 2);*/
